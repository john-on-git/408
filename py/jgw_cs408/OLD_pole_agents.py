...
#Replay method from Playing Atari with Deep Reinforcement Learning, Mnih et al (Algorithm 1).
class ParallelDQNAgent(Model):  
    def __init__(self, learningRate, discountRate, gradsCollector=None, lock=None, epsilon=0, epsilonDecay=1):
        super().__init__()
        self.gradsCollector = gradsCollector
        self.lock = lock
        self.epsilon = epsilon
        self.epsilonDecay = epsilonDecay
        self.discountRate = np.float32(discountRate)
    ...
    def train_step(self, x):
        ...
        with tf.GradientTape() as tape:
            s1, a, r, s2 = x    
            grads = tape.gradient(l(s1,a,r,s2), self.trainable_weights)
            self.gradsCollector.append(grads)
            self.optimizer.apply_gradients(zip(
                grads,
                self.trainable_weights
            ))
    ...
    def handleStep(self, endOfEpoch, observationsThisEpoch, actionsThisEpoch, rewardsThisEpoch, callbacks=[]):
        ...
        if len(observationsThisEpoch)>1: #if we have a transition to add
            self.train_step((observationsThisEpoch[-2], actionsThisEpoch[-2], rewardsThisEpoch[-2] + entropy(observationsThisEpoch), observationsThisEpoch[-1]))
            
            #do weight update
            #if endOfEpoch:
            #    for i in range(len(advantageThisEpoch)):
            #        advantageThisEpoch[i]+=entropy
            #    dataset = tf.data.Dataset.from_tensor_slices((observationsThisEpoch[:-1], actionsThisEpoch[:-1], rewardsThisEpoch[:-1], observationsThisEpoch[1:]))    
            #    self.fit(dataset, batch_size=10, callbacks=callbacks, verbose=False) #train on the minibatch