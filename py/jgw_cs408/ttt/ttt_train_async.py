...
def train(rngSeed, agentType, receivedWeights, sentGradients:Queue, lock, yss, running):
    ...
    gradsCollector = []
    agent = agentType(learningRate=0.001, discountRate=.95, epsilon=.1 , gradsCollector=gradsCollector)
    
    random.seed(rngSeed)
    tf.random.set_seed(rngSeed)
    np.random.seed(rngSeed)
    env = TTTEnv(opponent=SearchAgent(epsilon=.75))
...
if __name__ == "__main__":
    ...
    with Manager() as manager:
        ...
        agents[0].load_weights("jgw_cs408/checkpoints/TTTParallelDQNAgent.tf")
        ...
        for i in range(len(agents)):
            ...
            obs, _ = TTTEnv(opponent=SearchAgent()).reset()
